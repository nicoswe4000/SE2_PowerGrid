...
public class PlayerParticipatesTest {
    ...
    private final String fcqnrules="edu.hm.mrodic.powergrid.logic.MRodicRules";
    ...
    public void testNullGamePlayer() {
        ...
        Assert.assertTrue(rulesNull.getPrototypes().stream().map(Move::getType).anyMatch(type->type.equals(MoveType.JoinPlayer)));
    }
    ...
    public void testMaxPlayersFail() {
        ...
        Optional<Move> move=rules.getMoves(Optional.of("test5")).stream().filter(prototype->prototype.getType().equals(MoveType.JoinPlayer)).findFirst();
    ...
    public void testNullGameStart() {
        game.setPhase(Phase.Opening);
        final Rules rulesNull = Rules.newRules(fcqnrules,null);
        Assert.assertTrue(rulesNull.getMoves(Optional.of("test1")).stream().anyMatch(step->step.getType().equals(MoveType.CommenceGame)));
    }
    ...
    public void fireWorks2() {
        ...
        Assert.assertEquals(1, rules.getGame().getLevel());
        Assert.assertEquals(1, rules.getGame().getRound());
        Assert.assertEquals(Phase.PlayerOrdering, rules.getGame().getPhase());
    }
    ...
    public void notYourTurnTest() {
        ...
        game.getOpenPlayers().add(sep);
        game.setPhase(Phase.Opening);
        Optional<Move> move=rules.getMoves(Optional.of("test2")).stream().filter(option->option.getType().equals(MoveType.CommenceGame)).findFirst();
        Assert.assertEquals(Problem.NotYourTurn, rules.fire(Optional.of("test2"),move.get())); //gibt keinen Spieler mit secret test2, sollte Problem werfen
    }
    ...
    public void testInitialisePlantMarket() {
        ...
        Assert.assertFalse(game.getPlantMarket().findPlant(50) == null); //nimmt nur aus allPlants geht nicht so gut einzelne raus zu nehmen
    }
